...
- [x] **Step 2: 运行测试确认失败** → `npm test` FAIL。
...
- [x] **Step 4: 运行测试确认通过** → `npm test` PASS。
...
- [x] **Step 3: 实现 `relation.ts`**（无向 pair 去重，`source<target` 用字典序，id 用 `sha1(source|target)`）。
...
- [x] **Step 3: 实现 `cluster.ts`**（union-find 或 BFS 连通分量；输出确定性排序）。
...
- [x] **Step 3: 运行测试确认失败** → FAIL（repository 不存在）。

- [x] **Step 4: 实现 `repository.ts`**（用 M2 的 `insertArticles` 同样方式：`stmt.step`/`getRowsModified`；`mergeEdges` 用 `INSERT ... ON CONFLICT(id) DO UPDATE SET weight=weight+excluded.weight, last_seen_at=excluded.last_seen_at`，返回影响行数判定 upsert）。

- [x] **Step 5: 运行测试确认通过** → PASS；`npx tsc --noEmit` 无错。
...
- [x] **Step 3: 实现 `build.ts` 与 `defaultGazetteer`**。

- [x] **Step 4: 接 IPC**（contracts + register + index）。

- [x] **Step 5: 运行测试确认通过** → PASS；`npx tsc --noEmit` 无错；`npm run build` 成功。
...
## M3 交付记录（2026-09-14）
- Task 1 `ec54cd0`（双语词典抽取）· Task 2 `512d34c`（共现关系）· Task 3 `596d9cc`（事件聚类）· Task 4 `6377f9a`（图谱 schema v3+仓储）· Task 5 `c5ddeb9`（buildGraph 管线 + IPC）
- `npm test`：23/23 通过，`npx tsc` 无错，`npm run build` 成功，冒烟正常（sql.js 打开、IPC 就绪、3 源在列）。

## Rulings（裁定记录）
- **Ruling 5 — 实体 ID 用规范化小写名而非 sha1**：规范为 `id = name.toLowerCase()`（与 sha1 均确定性；小写名便于调试/可读）。若误判，代价是未来改名或歧义需迁移键。
- **Ruling 6 — 实体抽取用词典/句法匹配，暂不引重型 NLP**：v1 用双语 gazetteer 子串匹配，精度受限但零原生依赖、可跑可测；专业实体识别留待后续（M5 AI 介入）优化。
- **Ruling 7 — 关系类型 v1 仅 co-occurrence**：真正的因果/上下位关系延至 M5 AI 解读阶段产出。
